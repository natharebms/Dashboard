	...
	Set Rs = VScompDAAgencyFA.Get_RecordSet(CStr(STRSQL))
		...
		XX=0  	
			...
			while Not Rs.eof
				...
				Os_amount		=	RS("OS_AMOUNT")
				Location_id				=	RS("depT_code")
				If Trim(Amount) = "" Or isnull(Amount) Then Amount=0

				If TotString <> "" Then 
					TotString = TotString &","& "{""amount"": """&Os_amount&""", ""location"":"""&Location_id&"""}"
				Else 
					TotString = "{""amount"": """&Os_amount&""", ""location"":"""&Location_id&"""}"
				End If
					...
					<div id="MonthWiseDetail" class="panel-body panel-collapse collapse in">
					  ...
					  <div id="example" class="list-table">	
						...
						<ul class="list-head col-xs-12">
						  <li><div class="col-xs-6">Department</div><div class="col-xs-6">Amount in (Rs)</div></li>
						</ul>
				...
				new List('example', {
				  ...
				  pagination: true
					...
					var chart = AmCharts.makeChart("chartdiv", {
						...
						"balloonText": "[[title]]<br><span style='font-size:10px'><b>Rs.[[value]]</b></span>",
						"valueField": "amount",
						"titleField": "location",
						"startEffect": "elastic",
						"startDuration": 2,
